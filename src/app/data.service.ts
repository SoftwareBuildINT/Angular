...
export class DataService {
  private loginUrl = 'http://sbi-dashboard.hitachi.ifiber.in:7558/api/login'; // Login API URL
  ...
  getLhoList(lhoId: string): Observable<any> {
    const apiUrl = `http://sbi-dashboard.hitachi.ifiber.in:7558/api/lho-list?lho_id=${lhoId}`;
  ...
  addATM(atmData: { atmId: string; lho_id: string }): Observable<any> {
    const apiUrl = `http://sbi-dashboard.hitachi.ifiber.in:7558/api/add-atm`;
